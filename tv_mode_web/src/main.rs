...
async fn play_random_show_for_user(
    ...
    let selected_episode = {
        ...
        match client.select_random_episode_by_title(&selected_show_name).await {
            Ok(episode) => episode,
    ...
    // Play the selected episode (in another small scope)
    {
        ...
        match client.rpc_play(&selected_episode).await {
            Ok(_) => Ok(format!("Now playing {} - {}", selected_show_name, selected_episode)),
...
async fn start_tv_mode(
    ...
    let play_result = play_random_show_for_user(&data.rpc_client, &data.show_mappings.shows, &user).await;
